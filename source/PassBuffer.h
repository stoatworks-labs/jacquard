...
/**
    ...
    **It reallocates only when it has to.** Ensure() is called every frame and
    is a no-op unless the grid (Ends x Picks) changes.
    ...
    **It owns its filtering.** The one buffer here, the cell means, is data
    read back texel for texel and must never be filtered.
