...
and a control whose value never reaches the loom is dead just as quietly:
...
test card and reports any that made no difference at all.
    ...
    python3 tools/sweep.py [--binary build/jqtest] [--size WxH] [--jobs N]

Exit code 1 means something is dead.

------------------------------------------------------------------ the traps

**Thread Shading fades out below a few pixels a crossing**, on purpose: at
the default 160 ends a 320 x 180 frame has crossings of 2 px and the
shading is provably off. Its sweep runs at 40 ends.

**Thread Aspect only means anything while Picks is Auto** (0, the default).
Picks set to a number fixes the grid, and the aspect follows the frame.

**An option's range is its element count, an integer's is its real range.**
`jqtest --list` prints both for exactly this reason, and the ends are what
get swept.
...
#   _frames     how many frames to render (default 3)
#   _low/_high  the two positions to compare, when not the range's ends
CONTEXT = {
    # Below a few pixels a crossing the shading is faded out by design.
    "Thread Shading": {"Ends": 40},
    # Auto picks, which is the default, said explicitly.
    "Thread Aspect": {"Picks": 0},
}
...
def main():
    ...
    ap.add_argument("--binary", default=str(ROOT / "build" / "jqtest"))
    ...
    scratch = tempfile.mkdtemp(prefix="jqsweep")
