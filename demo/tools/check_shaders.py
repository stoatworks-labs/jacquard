...
`python3 demo/tools/sync_shaders.py`, never a hand edit of the page. Galvo's
check by way of teletext, in this repo's shape.

------------------------------------------------------------------- why

`demo/plugin.js` holds three GLSL bodies and so does `source/Shaders.cpp`, and
`demo/loom.js` holds the loom's cost constants, the control ranges, the swatch
cards and the option names. Two copies drift -- quietly, because a demo that
renders a *plausible* picture looks exactly like a demo that renders the right
one. The whole claim of the page is that it runs the plugin's own shaders, so
the claim needs something enforcing it. Nothing else can: `jqtest` drives the
real plugin class and has no idea this page exists, and verify.sh's glslc step
compiles the C++ copies and never looks at the JS ones.
...
2. Regenerates both generated blocks (`sync_shaders.py`'s shaders and
   ...
   that text, so a constant of Loom.h, a range of Controls.h, a swatch or an
   option name that moved in the C++ fails here too.

------------------------------------------------------------------- what it cannot

Nothing here checks the *ported* half. The k-means, the row programme, the
loom and the frame sequence in loom.js and plugin.js are a hand translation of
Palette.cpp, Encoder.cpp, Loom.cpp, Weave.cpp, Controls.cpp and
Jacquard::ProcessOpenGL, and only a reader can tell whether they still agree.
When you change one of those, change it there too -- a wrong port shows up on
the page as a cloth that is subtly wrong, which nobody will notice.
"""
import os
import re
import sys
